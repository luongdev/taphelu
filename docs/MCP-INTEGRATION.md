...
Kiro has two hook surfaces. `dl install --runtime kiro` generates CLI agent JSON files with embedded hooks for `kiro-cli`, and IDE `.kiro.hook` files so the Kiro Agent Hooks panel can discover Taphelu hooks.
