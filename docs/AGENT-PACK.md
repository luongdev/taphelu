...
Kiro uses skill-based slash commands, generated CLI agents, and IDE hook files only for local workspace installs because the Kiro panel discovers hooks from `.kiro/hooks`. Gemini gets an extension under `extensions/taphelu` plus generated commands. Codex gets skills, MCP config, and compact instruction/status fallback files.
