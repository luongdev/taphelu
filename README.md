...
Runtime adapters target Claude, Kiro, Codex, and Gemini from the same canonical pack. `full-auto` install generates MCP config, skills, agents or role fallbacks, slash commands, hooks, and runtime-appropriate status integration. Claude gets native `statusLine`; Gemini keeps native footer model/context visible; Kiro gets CLI agent hooks plus IDE `.kiro.hook` files and uses `/dl-status` for Taphelu-specific state where custom statusline support is not available.
