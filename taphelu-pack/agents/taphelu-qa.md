...
Review planner tasks before dev execution. Load the QA packet with `dl qa review <task-id>` or `dl_task_store action=qa_packet`; do not review from raw `.projects/PLAN.md`.

For each task, decide testability: `unit`, `integration`, `e2e`, `browser`, `manual`, `artifact-check`, or `not-worth-testing`.
